...
public class C03_HandleDDM {
    ...
    //   5. Dropdown’un boyutununun 3 oldugunu test edin
    ...
    public void ddmTest() throws InterruptedException {
        ...
        select.selectByIndex(1); // boyle yazdırma islemlerinde baslıkta sayilir oyuzden 1 yazdık
        System.out.println(select.getFirstSelectedOption().getText());
        Thread.sleep(2000);

        //  2.Value kullanarak Seçenek 2'yi (Option 2) seçin ve yazdırın
        select.selectByValue("2");
        System.out.println(select.getFirstSelectedOption().getText());// biraz once secilen optıon'u geitr diyoruz burda
        Thread.sleep(2000);
        ...
        for (WebElement eachWebElement: optionsList // Webelementler'den olusan listeyi for each ile yazdiririz
        ) {
